...
- (NSString *)formatLogMessage:(DDLogMessage *)logMessage
{
    ...
    //在日志文件中为每条日志增加文件名、方法名、行号
    NSString *filePath = [NSString stringWithFormat:@"%s", (logMessage->file)];
    NSRange range1 = [filePath rangeOfString:@"/" options:NSBackwardsSearch];
    NSRange range2 = [filePath rangeOfString:@"." options:NSBackwardsSearch];
    range1.location++;
    range1.length = range2.location - range1.location;
    NSString *fileName = [filePath substringWithRange:range1];
    
    return [NSString stringWithFormat:@"%@ %x [%@ %s:%d] %@", dateAndTime, logMessage->machThreadID, fileName, logMessage->function, logMessage->lineNumber, logMessage->logMsg];
}
