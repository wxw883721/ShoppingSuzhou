...
- (void)logMessage:(DDLogMessage *)logMessage
{
	...
	if (logMsg)
	{
		...
		else
		{
			...
			char tidCStr[9];
			int tidLen = snprintf(tidCStr, 9, "0x%x", logMessage->machThreadID);
            ...
            //日志中增加文件名和方法名，提高日志可读性
            const char *fileName = logMessage->file;
            const char *func = logMessage->function;
            int i, pos;
            ...
            pos++;
#define LOG_FILE_METHOD_MAX_LEN 128
            char fileAndMethod[LOG_FILE_METHOD_MAX_LEN + 16] = {0};
            fileAndMethod[0] = '[';
            for(i = 1; i < LOG_FILE_METHOD_MAX_LEN; i++)
            {
                if(fileName[pos] == '.' || fileName[pos] == '\0')
                    break;
                fileAndMethod[i] = fileName[pos++];
            }
            fileAndMethod[i++] = ' ';
            
            //function
            for(pos = 0; pos < strlen(func) && i < LOG_FILE_METHOD_MAX_LEN; pos++)
            {
                fileAndMethod[i++] = func[pos];
            }
            fileAndMethod[i++] = ':';
            
            //line number
            char line[9];
			int lineLen = snprintf(line, 9, "%d", logMessage->lineNumber);
            for(pos = 0; pos < lineLen && i < LOG_FILE_METHOD_MAX_LEN; pos++)
            {
                fileAndMethod[i++] = line[pos];
            }
            
            fileAndMethod[i++] = ']';
            fileAndMethod[i++] = ' ';
            
            v[8].iov_base = (char *)fileAndMethod;
			v[8].iov_len = strlen(fileAndMethod);
