...
- (void)cancelOnRequestThread
{
	...
	DDLogVerbose(@"Request cancelled: %@",self);
	#endif
...
- (void)startSynchronous
{
	...
	DDLogVerbose(@"Starting synchronous request %@",self);
#endif
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	DDLogVerbose(@"Starting asynchronous request %@",self);
#endif
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([self connectionInfo]) {
			...
			// Check if we should have expired this connection
				...
				DDLogVerbose(@"Not re-using connection #%i because it has expired",[[[self connectionInfo] objectForKey:@"id"] intValue]);
				#endif
		...
		#if DEBUG_PERSISTENT_CONNECTIONS
		DDLogVerbose(@"Request #%@ will use connection #%i",[self requestID],[[[self connectionInfo] objectForKey:@"id"] intValue]);
		#endif
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				if ([self totalBytesSent] == [self postLength]) {
					DDLogVerbose(@"Request %@ finished uploading data",self);
				}
				#endif
			}
		}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	DDLogVerbose(@"Request finished: %@",self);
#endif
	if ([self error] || [self mainRequest]) {
		return;
	}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	DDLogVerbose(@"Request %@: %@",self,(theError == ASIRequestCancelledError ? @"Cancelled" : @"Failed"));
#endif
	[self setComplete:YES];
	...
	if (theError && [theError code] != ASIAuthenticationErrorType && [theError code] != ASITooMuchRedirectionErrorType) {
		[connectionsLock lock];
		#if DEBUG_PERSISTENT_CONNECTIONS
		DDLogVerbose(@"Request #%@ failed and will invalidate connection #%@",[self requestID],[[self connectionInfo] objectForKey:@"id"]);
		#endif
...
- (void)readResponseHeaders
{
	...
	#if DEBUG_REQUEST_STATUS
	if ([self totalBytesSent] == [self postLength]) {
		DDLogVerbose(@"Request %@ received response headers",self);
	}
	#endif		
	...
	if ([self shouldRedirect] && [responseHeaders valueForKey:@"Location"]) {
		if (([self responseStatusCode] > 300 && [self responseStatusCode] < 304) || [self responseStatusCode] == 307) {
			...
			#if DEBUG_REQUEST_STATUS
				DDLogVerbose(@"Request will redirect (code: %i): %@",[self responseStatusCode],self);
			#endif
			...
			if (![connectionHeader isEqualToString:@"close"]) {
				...
				if (keepAliveHeader) { 
					...
					if (max > 5) {
						...
						#if DEBUG_PERSISTENT_CONNECTIONS
							DDLogVerbose(@"Got a keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
						#endif					
					}
				...
				// Otherwise, we'll assume we can keep this connection open
					...
					[self setConnectionCanBeReused:YES];
					#if DEBUG_PERSISTENT_CONNECTIONS
						DDLogVerbose(@"Got no keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
					#endif
				}
			}
		}
...
{	
...
#if DEBUG_REQUEST_STATUS
	DDLogVerbose(@"Request %@ finished downloading data (%qu bytes)",self, [self totalBytesRead]);
#endif
	[self setStatusTimer:nil];
	...
	#if DEBUG_PERSISTENT_CONNECTIONS
	DDLogVerbose(@"Request #%@ finished using connection #%@",[self requestID], [[self connectionInfo] objectForKey:@"id"]);
	#endif
	[[self connectionInfo] removeObjectForKey:@"request"];
	[[self connectionInfo] setObject:[NSDate dateWithTimeIntervalSinceNow:[self persistentConnectionTimeoutSeconds]] forKey:@"expires"];
	[connectionsLock unlock];
...
- (BOOL)retryUsingNewConnection
{
	if ([self retryCount] == 0) {
		#if DEBUG_PERSISTENT_CONNECTIONS
			DDLogVerbose(@"Request attempted to use connection #%@, but it has been closed - will retry with a new connection", [[self connectionInfo] objectForKey:@"id"]);
		#endif
		[connectionsLock lock];
		[[self connectionInfo] removeObjectForKey:@"request"];
		[persistentConnectionsPool removeObject:[self connectionInfo]];
	...
	#if DEBUG_PERSISTENT_CONNECTIONS
		DDLogVerbose(@"Request attempted to use connection #%@, but it has been closed - we have already retried with a new connection, so we must give up", [[self connectionInfo] objectForKey:@"id"]);
	#endif	
	return NO;
}
...
#if DEBUG_PERSISTENT_CONNECTIONS
			DDLogVerbose(@"Closing connection #%i because it has expired",[[existingConnection objectForKey:@"id"] intValue]);
#endif
		...
		if (throttleWakeUpTime) {
			if ([throttleWakeUpTime timeIntervalSinceDate:[NSDate date]] > 0) {
				if ([self readStreamIsScheduled]) {
					[self unscheduleReadStream];
					#if DEBUG_THROTTLING
					DDLogVerbose(@"Sleeping request %@ until after %@",self,throttleWakeUpTime);
					#endif
				}
			} else {
				if (![self readStreamIsScheduled]) {
					[self scheduleReadStream];
					#if DEBUG_THROTTLING
					DDLogVerbose(@"Waking up request %@",self);
					#endif
				}
			}
		} 
...
+ (void)recordBandwidthUsage
{
	...
	#if DEBUG_THROTTLING
	//DDLogVerbose(@"===Used: %u bytes of bandwidth in last measurement period===",bandwidthUsedInLastSecond);
	#endif
    ...
    //add by liuleting 14-2-25 begin 防止除数为0
    if (0 == measurements)
    {
        measurements=1;
    }
    //add by liuleting 14-2-25 begin
